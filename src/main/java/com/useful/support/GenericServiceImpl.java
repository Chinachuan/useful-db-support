...
public class GenericServiceImpl implements GenericService {
	...
	@Transactional(rollbackFor=Exception.class)
	public void adjustTableAutoId(String[] table) throws DbException {
		try{
			
			String sql = "SELECT max(id) FROM "+table[1];
			List<Object> list = genericDao.findBySql(sql, null);
			long newId=1;
			if(list != null && list.size()>0 && list.get(0)!=null){
				newId =Long.parseLong(list.get(0)+"");
			}
			if(newId<IdEntity.AUTO_ID) newId=IdEntity.AUTO_ID;
			newId++;
			sql = "ALTER TABLE "+table[0]+" AUTO_INCREMENT="+newId;
			genericDao.updateBySql(sql, null);
		}catch(Exception e){
			throw new DbException("调整表自增ID异常",e);
		}
	}
